            ...
            <dl className="mt-6 space-y-4">
                {order.orderProducts.map(product => (
                    <div 
                        ...
                        className="flex items-center gap-2 border-gray-200 pt-4 justify-between"
                    >
                        <dt className="flex items-center text-sm text-gray-600">
                            <span> ({product.quantity}) {''} </span>
                        </dt>
                        <dd className="text-sm font-medium text-gray-900" > {product.product.name} </dd>
                        <dd className="text-sm font-medium text-gray-900" > { formatCurrency(product.product.price * product.quantity)}  </dd>
                    </div>
                ))}
